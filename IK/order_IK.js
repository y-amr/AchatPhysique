...
const create_record = async (path, data, order_number) => {
  //console.log(data)
  const csvWriter = createCsvWriter({
    ...
    header: [
      ...
      {id: 'FIRST_NAME_SHIPPING', title: 'FIRST_NAME_SHIPPING'},
      {id: 'LAST_NAME_SHIPPING', title: 'LAST_NAME_SHIPPING'},
...
const updateOrderStatus = async (path, order_number, newStatus) => {
  const orders = [];
...
const updateOrderContent = async (order_number, number_pack,number_exclusive,number_standard,price) => {
  const orders = [];
  ...
  fs.createReadStream(path)
    ...
    .on('data', (row) => {
      ...
      if(row['ORDER_NUMBER'] === String(order_number)) {  // Use trimmed header name
        ...
        row['NUMBER_STANDARD'] = number_standard;  // Use trimmed header name_card
